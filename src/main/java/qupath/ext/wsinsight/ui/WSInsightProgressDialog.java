...
public class WSInsightProgressDialog {
    ...
    public WSInsightProgressDialog(String title, DockerRunner runner) {
        ...
        // If the user closes the window while the job is still running,
        // kill the container so we don't leave orphans behind.
        stage.setOnCloseRequest(ev -> {
            if (exitCode == null) {
                new Thread(runner::cancel, "wsinsight-docker-kill-on-close").start();
            }
        });
